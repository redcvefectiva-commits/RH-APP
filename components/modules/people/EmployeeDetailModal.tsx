...
const EmployeeDetailModal: React.FC<EmployeeDetailModalProps> = ({ isOpen, onClose, employee, allEmployees, onUpdateEmployee, onDeleteEmployee }) => {
  ...
  useEffect(() => {
    // This effect synchronizes the form data with the employee prop.
    // It runs only when the employee being viewed changes (i.e., employee.id changes).
    // This prevents the form from resetting while the user is typing, fixing the input bug.
  ...
  const renderInput = (id: keyof Employee, label: string, description?: string, type: string = 'text', required: boolean = false) => (
    <div>
  ...
  const renderSelect = (id: keyof Employee, label: string, options: readonly string[] | string[], description?: string, required: boolean = false) => (
    <div>
        <label htmlFor={id} className="block text-sm font-medium text-gray-700">{label}</label>
        {description && <p className="text-xs text-gray-500 mt-1">{description}</p>}
        <select
    ...
    const renderTextarea = (id: keyof Employee, label: string, description?: string, rows: number = 3) => (
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Perfil de ${employee.name}`} size="4xl">
      <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {activeTab === 'Personal' && (
                <Fragment>
                    <div className="flex items-center space-x-6">
                        <img 
                            src={formData.avatar || 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQp6_m5V-xG8MMmWJG_p-JYR_0-a9oMd6K24A&s'} 
                            alt="Avatar" 
                            className="w-24 h-24 rounded-full object-cover border-2 border-gray-200"
                        />
                        <div>
                            <label htmlFor="avatar-upload-edit" className="cursor-pointer bg-orange-100 text-primary px-3 py-2 rounded-md text-sm font-medium hover:bg-orange-200">
                                Cambiar Foto
                            </label>
                            <input id="avatar-upload-edit" name="avatar" type="file" className="hidden" accept="image/*" onChange={handleAvatarChange} />
                            ...
                            {Object.entries(documentsToUpload).map(([key, label]) => (
                                <div key={key} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                    ...
                                    {uploadedDocs[key] ? (
                                        ...
                                        <label htmlFor={`doc-${key}`} className="cursor-pointer bg-white border border-primary text-primary px-3 py-1 rounded-md text-sm font-medium hover:bg-orange-50">
                                            Adjuntar Archivo
                                        </label>
                                    )}
                                    <input id={`doc-${key}`} type="file" className="hidden" onChange={(e) => handleFileUpload(e, key)} />
                                </div>
                            ))}
                        </div>
                    </Section>
        ...
        <div className="flex justify-between items-center pt-4 mt-4 border-t">
          <div>
            <Button type="button" variant="danger" onClick={() => onDeleteEmployee(employee.id)}>Eliminar Empleado</Button>
          </div>
          <div className="flex">
